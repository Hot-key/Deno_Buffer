...
test({
    ...
    fn(): void {
        let data: string = "!#$%^ %godream1^^%@ Hello Tạm biệt ഗുഡ്ബൈ 高仁福 나라말싸미, 말싸밓, 말쌓밒";
        let Utf8StringToBytes = BitConverter.StringToBytes(data, StringType.Utf8);
        let BytesToStringUtf8 = BitConverter.BytesToString(Utf8StringToBytes, StringType.Utf8);
...
test({
    ...
    fn(): void {
        let data: string = "this is utf-16 string 한글도 가능 1234567!@#$%%^&}¢‡¥”w¯¥„ÈÉÞ´µ½²ÂÄÂÁ¾¿ÀÁÂ";
        let Utf16StringToBytes = BitConverter.StringToBytes(data, StringType.Utf16);
        let BytesToStringUtf16 = BitConverter.BytesToString(Utf16StringToBytes, StringType.Utf16);
...
test({
    ...
    fn(): void {
        let data: string = "!#$%^ %godream1^^%@ Hello Tạm biệt ഗുഡ്ബൈ 高仁福 나라말싸미, 말싸밓, 말쌓밒";
        let Utf8StringToBytes = BitConverter.StringToBytes(data, StringType.Utf8, true);
        let BytesToStringUtf8 = BitConverter.BytesToString(Utf8StringToBytes, StringType.Utf8);
...
test({
    ...
    fn(): void {
        let data: string = "this is utf-16 string 한글도 가능 1234567!@#$%%^&}¢‡¥”w¯¥„ÈÉÞ´µ½²ÂÄÂÁ¾¿ÀÁÂ";
        let Utf16StringToBytes = BitConverter.StringToBytes(data, StringType.Utf16, true);
        let BytesToStringUtf16 = BitConverter.BytesToString(Utf16StringToBytes, StringType.Utf16);
